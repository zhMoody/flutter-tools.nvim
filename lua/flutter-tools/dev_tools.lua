...
function M.open_dev_tools()
  ...
  if url then
    ...
    ui.notify("No active devtools server found", 1)
  end
end

function M.handle_log(data)
  if not data or #data == 0 then
    ui.notify("Received empty or nil log data", ui.ERROR)
    return
  end
...
local function handle_start(_, data, _)
  if not data or #data == 0 then
    ui.notify("Received empty or nil data from devtools startup", ui.WARN)
    return
  end
